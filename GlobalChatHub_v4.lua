...
local function fetchAvatar(uid, imgLbl)
    ...
    if avCache[uid] then
        pcall(function() imgLbl.Image = avCache[uid] end); return
    end
    task.spawn(function()
        task.wait(0.1) -- pequeno delay pra evitar flood
        local ok, url = pcall(function()
            return Players:GetUserThumbnailAsync(uid,
                Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48)
        end)
        if ok and url and type(url)=="string" and url~="" then
            avCache[uid] = url
            pcall(function()
                if imgLbl and imgLbl.Parent then imgLbl.Image = url end
            end)
        end
    end)
end
