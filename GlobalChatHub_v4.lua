...
-- ── HTTP Detection (sem teste de startup — evita CrossExperience spam) ──
-- Detecta só pela existência; erros são silenciados no doRequest()
local httpFn, httpName = nil, "none"
local useHttpSvc = false

local function _pick()
    if syn and type(syn.request)=="function" then return syn.request,"syn.request" end
    if type(http_request)=="function" then return http_request,"http_request" end
    if http and type(http.request)=="function" then return http.request,"http.request" end
    if fluxus and type(fluxus.request)=="function" then return fluxus.request,"fluxus.request" end
    if type(request)=="function" then return request,"request" end
    return nil,"none"
end
httpFn, httpName = _pick()

local function doRequest(opts)
    -- Tenta executor fn primeiro
    if httpFn then
        ...
        if ok and r then
            ...
            return {Success=(sc>=200 and sc<300), StatusCode=sc, Body=body}
        end
        -- Se falhou (CrossExperience etc.), cai para HttpService
    end
    -- Tenta HttpService
    local ok2, r2 = pcall(function()
        if opts.Method == "GET" then
            ...
            return Http:PostAsync(opts.Url, opts.Body or "",
                Enum.HttpContentType.ApplicationJson)
        end
    end)
    if ok2 and r2 then
        useHttpSvc = true; httpName = "HttpService"
        return {Success=true, StatusCode=200, Body=tostring(r2)}
    end
    return nil
end
