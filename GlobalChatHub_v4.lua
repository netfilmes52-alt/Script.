...
-- ── HTTP Detection (testa de verdade antes de usar) ────────
local httpFn, httpName = nil, "none"
local useHttpSvc = false

-- URL de teste simples que sempre retorna algo
local TEST_URL = FIREBASE_URL .. "/ping.json"

local function tryFn(fn)
    -- Faz uma requisição GET real pra confirmar que funciona
...
local candidates = {
    ...
    {n="http_request",   f=function() return typeof(http_request)=="function" and http_request end},
    {n="http.request",   f=function() return http and http.request end},
    {n="fluxus.request", f=function() return fluxus and fluxus.request end},
    {n="request",        f=function() return typeof(request)=="function" and request end},
}

for _, c in ipairs(candidates) do
    local ok, fn = pcall(c.f)
    if ok and fn then
        if tryFn(fn) then
            httpFn = fn; httpName = c.n; break
        end
    end
end

if not httpFn then
    -- Tenta HttpService do Roblox
    local ok, res = pcall(function() return Http:GetAsync(TEST_URL) end)
    if ok and res then
...
local function doRequest(opts)
    if useHttpSvc then
        local ok, r = pcall(function()
            ...
            elseif opts.Method == "DELETE" then
                -- HttpService não tem DELETE direto, usa RequestAsync se disponível
                return Http:GetAsync(opts.Url) -- fallback
            else
    ...
    if ok and r then
        -- Normaliza resposta
        local sc = tostring(r.StatusCode or r.status_code or "200")
        if sc == "200" or r.Success then
            return {Success=true, StatusCode=200, Body=r.Body or r.body or ""}
        end
        return {Success=false, StatusCode=tonumber(sc) or 0, Body=r.Body or r.body or ""}
    end
    return nil
end
...
local function fbRaw(method, path, data)
    ...
    -- 404 ou vazio = path não existe ainda (normal no Firebase)
    ...
    if code == 200 or res.Success then
        ...
        return {}, nil -- JSON malformado → trata como vazio
    end
...
-- ── Avatar cache ──────────────────────────────────────────
-- GetUserThumbnailAsync bloqueado no Delta (CrossExperience) → desabilitado
local avCache = {}
local function fetchAvatar(_uid, _imgLbl) end
